...
/**
 * DesktopPdfViewer.tsx
 *
 * Full-featured react-pdf viewer for desktop and Android.
 * This file is lazy-loaded and NEVER imported on iOS,
 * so pdfjs-dist is completely absent from the iOS bundle.
 */

import {
    ChevronRight, ChevronLeft, BookmarkPlus, BookmarkCheck,
    Maximize2, Minimize2, ExternalLink, Loader2, AlertCircle,
    ZoomIn, ZoomOut, Moon, Sun, SkipBack, SkipForward, Menu, X,
} from 'lucide-react';
...
export default function DesktopPDFViewer({
    ...
    return (
        <div
            ...
            style={{
                height: isMobile ? window.innerHeight : '82vh',
        ...
        >
            ...
            <div className={cn('relative z-30 flex items-center gap-2 border-b flex-shrink-0', toolbarBg, isMobile ? 'px-3 py-2.5' : 'px-4 py-2')}>
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                    ...
                    <TBtn onClick={() => goToPage(displayPage - 1, 'right')} disabled={displayPage <= 1} dark={isDark}>
                    ...
                    <TBtn onClick={() => goToPage(displayPage + 1, 'left')} disabled={!!numPages && displayPage >= numPages} dark={isDark}>
                ...
                <div className="flex-1 flex items-center justify-end gap-0.5">
                    ...
                    <TBtn onClick={() => setIsDark(d => !d)} dark={isDark}>
                        {isDark ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
                    </TBtn>
                    {!isMobile && supportsFullscreen && (
                        <TBtn onClick={toggleFullscreen} dark={isDark}>
                            {isFullscreen ? <Minimize2 className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
                        </TBtn>
                    )}
                    <TBtn onClick={() => window.open(url, '_blank', 'noopener,noreferrer')} dark={isDark}>
                    ...
                    {renderError && (
                        <div className="absolute inset-0 flex flex-col items-center justify-center z-10 gap-5 px-8">
                            ...
                            <div className="text-center">
                                ...
                                <button onClick={() => window.open(url, '_blank', 'noopener,noreferrer')} className="flex items-center gap-2 mx-auto px-5 py-3 bg-rose-600 hover:bg-rose-700 text-white text-sm rounded-xl font-medium">
                            ...
                            >
                                {isMobile ? (
                                    <div className="w-full flex justify-center" style={{
                                        ...
                                        {containerWidth > 0 && (
                                            <Page pageNumber={displayPage} scale={fitScale} renderTextLayer={false} renderAnnotationLayer={false} />
                                        )}
                                    </div>
                                ) : (
                                    numPages
                                            ...
                                            <div key={pageNum} ref={setPageRef(pageNum)} className="mb-6 flex justify-center">
                                                {windowedPages.has(pageNum) ? (
                                                    <div className="shadow-2xl">
                                                        <Page
                                                            pageNumber={pageNum} scale={effectiveScale}
                                                            renderTextLayer renderAnnotationLayer
                                                            onRenderSuccess={pageNum === 1 ? (pg: any) => setPageHeight(pg.height) : undefined}
                                                        />
                                                    </div>
                                                ) : (
                ...
                <div className={cn('flex-shrink-0 border-t relative z-30', toolbarBg)}>
                    ...
                    <div className="flex items-center px-4 py-3 gap-3">
                        <button onClick={() => goToPage(displayPage - 1, 'right')} disabled={displayPage <= 1}
                            className={cn('flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl text-sm font-semibold transition-all active:scale-95 select-none',
                        ...
                        <button onClick={() => { setShowPageInput(true); setPageInputVal(String(displayPage)); }} className="flex flex-col items-center justify-center px-2 min-w-[56px]">
            ...
            {isMobile && showPageInput && (
                <div className="absolute inset-0 z-50 flex items-end" style={{ background: 'rgba(0,0,0,0.55)' }} onClick={() => { setShowPageInput(false); setPageInputVal(''); }}>
                    <div className={cn('w-full rounded-t-3xl p-6 pb-10', isDark ? 'bg-[#1a1d24]' : 'bg-white')} onClick={e => e.stopPropagation()}>
                        ...
                        <input ref={pageInputRef} type="number" min={1} max={numPages ?? 9999} value={pageInputVal} onChange={e => setPageInputVal(e.target.value)} onKeyDown={e => e.key === 'Enter' && submitPageInput()} placeholder={`1 – ${numPages ?? '?'}`} autoFocus
                        ...
                        />
                        <button onClick={submitPageInput} className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-semibold text-base">Go to page</button>
                    </div>
                </div>
            )}
