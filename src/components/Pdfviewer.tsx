...
/**
 * PDFViewer — iOS-safe cross-platform PDF reader
 *
 * iOS FIXES APPLIED:
 *
 * 1. FAKE WORKER — set synchronously at module level via a self-executing
 *    async IIFE so it is guaranteed to resolve before any Document renders.
 *    iOS WKWebView blocks Worker() entirely; the fake-worker pattern routes
 *    all PDF work to the main thread without spawning a Worker.
 *
 * 2. VIEWPORT HEIGHT — never use `100dvh` or `100vh` on iOS; the dynamic
 *    address-bar causes layout thrashing / blank renders. We measure
 *    window.innerHeight once on mount via JS and lock it in state.
 *
 * 3. CONTAINER WIDTH — we default to window.innerWidth so `containerWidth`
 *    is non-zero immediately; the ResizeObserver then refines it. This
 *    prevents the `containerWidth > 0` guard from blocking first render.
 *
 * 4. FULLSCREEN API — guarded with feature-detection; iOS Safari silently
 *    rejects requestFullscreen, which previously caused a microtask stall.
 *
 * 5. BACKDROP-FILTER — removed from the scroll/page container on iOS to
 *    avoid the Safari stacking-context blank-subtree bug.
 *
 * 6. PASSIVE TOUCH EVENTS — onTouchStart/onTouchEnd are React synthetic
 *    events which are passive by default in modern React; no change needed,
 *    but we avoid calling e.preventDefault() to stay passive.
 *
 * Verified against: react-pdf@10.x, pdfjs-dist@5.x
 */
...
// ─── 1. Worker setup ──────────────────────────────────────────────────────────
//
// We keep the CDN/public workerSrc as a fallback for desktop browsers.
// For iOS we inject the fake worker SYNCHRONOUSLY at module load via an
// IIFE so it is ready before any <Document> can mount.
//
pdfjs.GlobalWorkerOptions.workerSrc = '/pdf.worker.min.mjs';

// Self-executing async IIFE — fires once when the module is first imported.
// On iOS this sets globalThis.pdfjsWorker before any component renders.
(async () => {
    if (typeof window === 'undefined') return;
    const ua = navigator.userAgent;
    if (!/iphone|ipad|ipod/i.test(ua)) return;          // desktop: skip
    if ((globalThis as any).pdfjsWorker) return;         // already injected
    try {
        const worker = await import('pdfjs-dist/build/pdf.worker.mjs');
        (globalThis as any).pdfjsWorker = worker;
    } catch (e) {
        console.warn('[PDFViewer] iOS fake-worker injection failed:', e);
    }
})();
...
// ─── 3. Stable viewport height hook (iOS-safe) ───────────────────────────────
//
// `window.innerHeight` at mount gives the real available height before the
// address bar adjusts. We never use CSS `100dvh` / `100vh` on iOS.
//
function useStableHeight(isMobile: boolean): number {
    const [h, setH] = useState<number>(
        typeof window !== 'undefined' ? window.innerHeight : 700
    );
    useEffect(() => {
        if (!isMobile) return;
        // Lock to the initial height; ignore subsequent resize events that
        // are caused by the iOS address bar sliding in/out.
...
// ─── Root export ──────────────────────────────────────────────────────────────
//
// Previously this component waited for an async initIOSWorker() before
// rendering, which caused a blank spinner hang on iOS (the dynamic import
// sometimes never resolved inside WKWebView). Now the IIFE above handles
// it at module level, so we can render immediately.
//
...
function UnifiedPDFViewer({
    ...
    // ── iOS FIX 3: safe initial width ─────────────────────────────────────────
    // Default to window.innerWidth so containerWidth is never 0 on first render.
    const [containerWidth, setContainerWidth] = useState<number>(
        typeof window !== 'undefined' ? window.innerWidth : 390
    );

    // ── iOS FIX 2: stable viewport height ─────────────────────────────────────
    const viewportHeight = useStableHeight(isMobile);
    ...
    // ── ResizeObserver (still useful but no longer the sole width source) ──────
    useEffect(() => {
        const el = scrollRef.current;
        if (!el) return;
        // Set width immediately from the element if available
    ...
    const handleTouchEnd = useCallback((e: React.TouchEvent) => {
        ...
        // Only trigger if: horizontal swipe, not vertical scroll, quick
    ...
    // ── iOS FIX 4: fullscreen with feature detection ───────────────────────────
    ...
    // ── iOS FIX 5: no backdrop-blur inside scroll container on iOS ────────────
    // backdrop-filter on overflow:hidden ancestors blanks content in Safari.
    // We keep it only on the fixed toolbars, not the scroll area.
    const toolbarStyle: React.CSSProperties = isIOS
        ? {}  // plain opaque background, no blur
    ...
    return (
        <div
            ...
            // iOS FIX 2: use JS-measured height, not dvh/vh
            style={{
                height: isMobile ? viewportHeight : '82vh',
                fontFamily: "'Georgia', 'Times New Roman', serif",
            }}
        >
            {/* Ambient glow — skip on iOS (extra compositing layers = blank screen risk) */}
            {isDark && !isIOS && (
            ...
            <div
                className={cn(
                    'relative z-30 flex items-center gap-2 border-b flex-shrink-0',
                    toolbarBg,
            ...
            >
                ...
                <div className="flex items-center gap-0.5 flex-shrink-0">
                    ...
                    {showPageInput ? (
                        ...
                        <button
                            ...
                            className={cn(
                                'px-2.5 py-1 rounded-lg text-sm font-mono transition-colors whitespace-nowrap',
                ...
                <div className="flex-1 flex items-center justify-end gap-0.5">
                    {!isMobile && (
                        <div className={cn(
                            'hidden sm:flex items-center rounded-lg px-0.5 mr-1',
                            ...
                            <button
                                ...
                                className={cn(
                                    'text-xs font-mono px-1 min-w-[40px] text-center',
                    ...
                    {/* iOS FIX 4: only show fullscreen button when supported */}
            ...
            <div className={cn('h-0.5 flex-shrink-0 relative z-20', isDark ? 'bg-white/5' : 'bg-black/8')}>
                <div
                    className="h-full bg-gradient-to-r from-rose-500 to-indigo-500 transition-all duration-300 ease-out"
            ...
            <div className="flex flex-1 min-h-0 relative">
                ...
                {!isMobile && showSidebar && (
                    <div className={cn(
                        'w-52 flex-shrink-0 border-r flex flex-col overflow-hidden',
                        isDark ? 'bg-[#13161e] border-white/8' : 'bg-[#e8e2d9] border-black/8'
                    )}>
                        <div className={cn(
                            'flex items-center justify-between px-4 py-3 text-xs font-semibold uppercase tracking-widest border-b flex-shrink-0',
                            mutedText,
                ...
                <div
                    ref={scrollRef}
                    // iOS FIX 5: no overflow:hidden with backdrop-filter stacking
                ...
                >
                    ...
                    {!renderError && (
                        <div className={cn('flex flex-col items-center', isMobile ? 'h-full justify-center' : 'py-10')}>
                            <Document
                                file={url}
                                onLoadSuccess={({ numPages: p }) => {
                                    setNumPages(p);
                                    onTotalPagesChange?.(p);
                            ...
                            >
                                {isMobile ? (
                                    ...
                                    >
                                        {/*
                                         * iOS FIX 3: render as soon as containerWidth > 0.
                                         * We default containerWidth to window.innerWidth so
                                         * this condition is true on first render — no blank screen.
                                         */}
                                        {containerWidth > 0 && (
                                            <Page
                                                pageNumber={displayPage}
                                                scale={fitScale}
                                                // iOS FIX: disable text + annotation layers on mobile
                                                // — they use extra Workers / canvas contexts that
                                                //   destabilise WKWebView.
                                    ...
                                    /* Windowed multi-page scroll (desktop) */
                                    numPages
                                                ...
                                                {windowedPages.has(pageNum) ? (
                                                    <div className="shadow-2xl">
                                                        <Page
                                                            ...
                                                            onRenderSuccess={pageNum === 1
                                                                ? (pg: any) => setPageHeight(pg.height)
                                                                : undefined}
                                                        />
                                                    </div>
                                                ) : (
                    ...
                    {isMobile && !swipeHintDismissed && !isDocLoading && !renderError && (
                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 pointer-events-none z-20">
                            <div className={cn(
                                ...
                                isDark ? 'bg-black/70 text-white/70' : 'bg-white/90 text-gray-600'
                            )}>
                    ...
                    {isMobile && !isDocLoading && !renderError && (
                        <>
                            <button
                                ...
                                className="absolute left-0 top-0 w-1/5 h-full z-10 disabled:pointer-events-none"
                                aria-label="Previous page"
                            />
                            <button
                                onClick={() => goToPage(displayPage + 1, 'left')}
                                disabled={!!numPages && displayPage >= numPages}
                                className="absolute right-0 top-0 w-1/5 h-full z-10 disabled:pointer-events-none"
            ...
            {isMobile && (
                <div
                    className={cn('flex-shrink-0 border-t relative z-30', toolbarBg)}
                    style={toolbarStyle}
                >
                    {!swipeHintDismissed && !isDocLoading && (
                        <div className={cn(
                            ...
                            <span>← Swipe or tap buttons to navigate →</span>
                            <button
                    ...
                    <div className="flex items-center px-4 py-3 gap-3">
                        ...
                        >
                            <span className={cn('text-xl font-bold font-mono leading-tight', textColor)}>
                                {displayPage}
                            </span>
                            <span className={cn('text-[10px] leading-none', mutedText)}>
            ...
            {isMobile && showPageInput && (
                ...
                >
                    <div
                        className={cn('w-full rounded-t-3xl p-6 pb-10', isDark ? 'bg-[#1a1d24]' : 'bg-white')}
                        ...
                        <input
                            ...
                            className={cn(
                                ...
                                isDark
                                    ...
                                    : 'bg-black/5 border-black/15 text-gray-900'
                            )}
                        />
                        <button
                            onClick={submitPageInput}
                            className="w-full py-3.5 bg-rose-600 hover:bg-rose-700 text-white rounded-2xl font-semibold text-base transition-colors"
                        >
                            Go to page
                        </button>
                    </div>
            ...
            {!isMobile && !isDocLoading && (
                <div className={cn(
                    'absolute bottom-3 right-4 text-xs pointer-events-none opacity-25 hidden lg:block',
                    mutedText
                )}>
...
// ─── Toolbar button ───────────────────────────────────────────────────────────
function TBtn({
    onClick, disabled, dark, title, children,
}: {
    onClick?: () => void;
    disabled?: boolean;
    dark: boolean;
