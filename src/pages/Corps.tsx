...
const UnionTeam = () => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      // No need to filter manually — RLS handles it
  ...
  const goNext = useCallback(() => {
    goTo((activeIndex + 1) % shuffled.length);
  }, [activeIndex, shuffled.length, goTo]);

  const goPrev = useCallback(() => {
    goTo((activeIndex - 1 + shuffled.length) % shuffled.length);
  }, [activeIndex, shuffled.length, goTo]);
  ...
  return (
    <div className="min-h-screen bg-background pb-24" dir="rtl">
      ...
      <style>{`
        ...
        .spotlight-card {
          animation: fadeSlideIn 0.4s cubic-bezier(.22,1,.36,1) both;
        }
        .spotlight-card.leaving {
          animation: fadeSlideOut 0.35s ease-in both;
        }
        .avatar-main {
          animation: avatarPop 0.5s cubic-bezier(.22,1,.36,1) both;
        }
        .ring-pulse {
          animation: ringPulse 2.4s ease-in-out infinite;
        }
        .dot-active {
          animation: dotBounce 0.4s ease-in-out both;
        }
        .thumb-avatar {
      ...
      <div className="px-4 py-6 max-w-lg mx-auto space-y-6">
        ...
        {loading ? (
          ...
          <div
            key={activeMember.id}
            className={`spotlight-card relative rounded-2xl overflow-hidden shadow-lg border border-border bg-card`}
          >
            {/* Decorative top bar */}
            ...
            <div className="flex flex-col items-center gap-4 p-8 text-center">
              ...
              <div className="relative">
                ...
                {/* Small equal-sign badge */}
                {/* <span className="absolute -bottom-1 -left-1 w-7 h-7 rounded-full bg-primary text-white text-xs flex items-center justify-center font-bold shadow-md select-none">
                  =
                </span> */}
              </div>
            ...
            <div className="flex justify-between items-center px-4 pb-5">
              <button
                onClick={goPrev}
                className="nav-btn w-10 h-10 rounded-full bg-muted hover:bg-muted/80 flex items-center justify-center text-foreground"
                aria-label="Previous"
              >
                ‹
              </button>
        ...
        {/* Thumbnail row — all members, no order implied */}
        {shuffled.length > 0 && (
          <div>
        ...
        <div className="rounded-xl border border-border bg-muted/40 p-4 text-center text-muted-foreground">
          <h4 className="font-semibold text-sm mb-1">تعرف على فريق عملنا</h4>
