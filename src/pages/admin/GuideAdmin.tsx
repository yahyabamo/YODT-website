    ...
    return (
        <div className="p-2">
            <div className="flex justify-between items-center mb-6">
                {/* Added text-slate-900 for high visibility */}
                <h2 className="text-xl font-black text-slate-900 dark:text-white">إدارة الدليل والأسئلة</h2>
            ...
            {/* Tab Switcher */}
            <div className="flex gap-2 mb-6 bg-white p-1.5 rounded-2xl shadow-sm w-fit dark:bg-slate-800">
            ...
            {loading ? <Spinner /> : (
                <div className="grid gap-4">
                    {items.filter(i => i.type === activeTab).map(item => (
                        <Card key={item.id} className="p-4 flex justify-between items-center bg-white border-border rounded-2xl dark:bg-slate-900">
                            <div className="flex items-center gap-4">
                                {activeTab === 'guide' && <span className="text-2xl">{item.icon}</span>}
                                {/* Added text-slate-800 to make the title clearly visible */}
                                <div className="font-bold text-slate-800 dark:text-slate-100">{item.title}</div>
                            </div>
                            <div className="flex gap-2">
                                <button onClick={() => { setEditing(item); setForm(item); setModal(true); }} className="w-9 h-9 flex items-center justify-center rounded-xl bg-slate-50 border-none cursor-pointer hover:bg-slate-100 transition-colors">✏️</button>
                                ...
                                })} className="w-9 h-9 flex items-center justify-center rounded-xl bg-red-50 text-red-500 border-none cursor-pointer hover:bg-red-100 transition-colors">🗑️</button>
                            </div>
                        </Card>
                    ))}
                </div>
