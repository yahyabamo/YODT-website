...
export default function AdminAcademyPage() {
    ...
    async function togglePublish(course: Course) {
        ...
        const { error } = await supabase
            .from('courses').update({ is_published: !course.is_published }).eq('id', course.id)
        if (!error) {
            setCourses(prev => prev.map(c => c.id === course.id ? { ...c, is_published: !c.is_published } : c))
            setStats(prev => ({ ...prev, published: prev.published + (course.is_published ? -1 : 1) }))
        }
        setTogglingId(null)
    }
    ...
    return (
        <div className="min-h-screen pb-20" style={{ background: '#F8F7F5', fontFamily: "'Cairo', sans-serif" }} dir="rtl">
            ...
            {/* ── Header ── */}
            <div className="px-4 md:px-8 py-6 md:py-8 relative overflow-hidden"
                ...
                <div className="relative flex items-start justify-between max-w-7xl mx-auto gap-4">
                    <div>
                        <p className="text-xs font-bold mb-1" style={{ color: 'rgba(220,38,38,0.8)' }}>لوحة الإدارة</p>
                        <h1 className="text-xl md:text-2xl font-black text-white mb-1">إدارة الأكاديمية</h1>
                    ...
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2.5 md:px-5 md:py-3 rounded-2xl font-bold text-sm text-white transition-all duration-200 hover:-translate-y-0.5 flex-shrink-0"
            ...
            <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 md:pt-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 mb-6 md:mb-8">
                    {[
                        ...
                        <div key={lbl} className="bg-white p-4 md:p-5 flex items-center gap-3 md:gap-4"
                            ...
                            <div>
                                <p className="text-lg md:text-xl font-black text-gray-900">{val}</p>
                ...
                <div className="flex flex-wrap gap-3 mb-5 items-center">
                    <div className="relative flex-1" style={{ minWidth: '160px', maxWidth: '320px' }}>
                        ...
                        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث..."
                            ...
                            style={{ background: '#fff', border: '1.5px solid #E5E7EB', borderRadius: '12px', color: '#111' }} />
                    </div>
                    <div className="flex gap-1">
                        {(['all', 'published', 'draft'] as const).map(f => (
                            <button key={f} onClick={() => setFilter(f)}
                                className="px-3 md:px-4 py-2.5 rounded-xl text-xs font-bold transition-all duration-150"
                                style={filter === f
                ...
                {loading ? (
                    ...
                    <>
                                ...
                                <thead>
                                    <tr style={{ borderBottom: '1px solid #F5F2EE' }}>
                                        {['الكورس', 'المدرب', 'المدة', 'الحالة', 'الإجراءات'].map(h => (
                                            <th key={h} className="text-right text-xs font-bold text-gray-400 px-5 py-4">{h}</th>
                                        ))}
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        <tr key={course.id}
                                            ...
                                            <td className="px-5 py-4">
                                                <span className="text-sm text-gray-500">
                                                    {course.duration_mins < 60 ? `${course.duration_mins} د` : `${Math.floor(course.duration_mins / 60)} س`}
                                                </span>
                                            </td>
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-1">
                                                    <ActionBtn icon={<Pencil className="w-3.5 h-3.5" />} label="تعديل"
                                                        onClick={() => navigate(`/admin/academy/${course.id}`)} color="#3B82F6" />
                                                    <ActionBtn icon={<Trash2 className="w-3.5 h-3.5" />} label="حذف"
                                                        onClick={() => deleteCourse(course.id)} color="#EF4444"
                                                        loading={deletingId === course.id} />
                                                    <ActionBtn icon={<Users className="w-3.5 h-3.5" />} label="الطلاب"
                                                        onClick={() => navigate(`/admin/academy/${course.id}/students`)} color="#10B981" />
                                                </div>
                                            </td>
                                        </tr>
                                    ))}
                        ...
                        <div className="md:hidden space-y-3">
                            {filtered.map(course => (
                                <div key={course.id} className="bg-white p-4"
                                    style={{ borderRadius: '16px', border: '1px solid #F0EDE8' }}>
                                    ...
                                    <div className="flex gap-3 mb-4">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-bold text-gray-900 line-clamp-2 leading-snug">{course.title}</p>
                                    ...
                                    <div className="flex items-center justify-between pt-3"
                                        ...
                                        <div className="flex items-center gap-2">
                                            ...
                                            >
                                                ...
                                                <span>{deletingId === course.id ? '...' : 'حذف'}</span>
                                            </button>
                                        </div>
                                    </div>
                                </div>
