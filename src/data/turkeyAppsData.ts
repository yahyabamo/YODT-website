...
{ app.is_essential && <Star className="w-3 h-3 text-warning fill-warning" />}
  ...
  < p className = "text-xs text-muted-foreground" > { app.name_tr } </p>
    </div>
    </div>
...
const TurkeyApps = () => {
  ...
  {/* Essential Apps horizontal scroll */ }
  {
...
className = {`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-all flex items-center gap-2 ${selectedCategory === cat.id ? 'bg-primary text-primary-foreground' : 'bg-secondary text-muted-foreground hover:bg-secondary/80'
  }`}>
